...
function normalizePrimaryNavigation(){
  ...
  var desktop=document.querySelector('.desktop-nav');if(desktop&&!desktop.dataset.complete){desktop.dataset.complete='true';var groups=[['Visão',['dashboard','Hoje'],['analysis','Análise']],['Operação',['register','Movimentos']],['Patrimônio',['wallet','Investimentos'],['debts','Dívidas'],['dividends','Rendimentos'],['simulator','Simulador']],['Planejamento',['plan','Planos'],['projection','Futuro'],['decisions','Decidir'],['weekly','Semana']],['Conta',['profile','Perfil'],['categories','Categorias'],['settings','Configurações'],['help','Ajuda']]];desktop.innerHTML=groups.map(function(group){return'<div class="nav-section"><div class="nav-label">'+group[0]+'</div>'+group.slice(1).map(function(item){return'<button data-view="'+item[0]+'"><span class="ico" aria-hidden="true"></span>'+item[1]+'</button>'}).join('')+'</div>'}).join('');var current=document.querySelector('.section.active');var active=current&&current.id||'dashboard';desktop.querySelectorAll('button[data-view]').forEach(function(button){button.classList.toggle('active',button.dataset.view===active)})}
