...
const affordable=engine.evaluatePurchase({amount:300,income:5000,workHours:176,availableBalance:3000,minimumReserve:1000,annualReturn:8,reason:'necessidade',priority:'alta',recurrence:'none',baseDecision:stable});
assert.equal(affordable.tag,'Compra defensável');
const unsafe=engine.evaluatePurchase({amount:1800,income:5000,workHours:176,availableBalance:2000,minimumReserve:1000,annualReturn:8,reason:'conforto',priority:'media',recurrence:'none',baseDecision:engine.evaluate({...base,cash:-400})});
assert.equal(unsafe.tag,'Esperar');
assert.equal(unsafe.basePriority,'corrigir_fluxo_atual');
assert.match(unsafe.formulaSimplificada,/saldo após compra/);
